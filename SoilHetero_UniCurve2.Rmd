...
$$ \frac{\mu(\frac{1}{\phi}+1)}{\mu(\frac{1}{\phi}+1)+t}^{(\frac{1}{\phi}+2)}$$

To be extremely clear, the amount of carbon from a given cohort of carbon atoms remaining at time $t$ is given by (5): 
$$ C(t) = C(0)\frac{\mu(\frac{1}{\phi}+1)}{\mu(\frac{1}{\phi}+1)+t}^{(\frac{1}{\phi}+2)}$$
Thus, given a time series of observations of turnover (e.g. from incubation experiments such as Schaedel et al. 2020), we could estimate the relevant parameters governing the distribution of residence times $\mu$ and $\phi$ inversely by fitting to equation (6). Alternately, we can use forward simulation results from a suitable PBM that provides such a distribution (e.g. Waring et al. 2020), plug in the resulting parameters and use (6) to estimate scaled-up properties and services. 
...
```{r Figure 1: universal bounding curves, echo = F}
...
$$ C_{avoided}(t) = C(0)\mu(1-(\frac{\mu(\frac{1}{\phi}+1)}{\mu(\frac{1}{\phi}+1)+t})^{(\frac{1}{\phi}+1)})$$
...
$$ C_{avoided}(t) = C(0)\mu(1-(\frac{1}{1+\tau}))$$
where $\tau$ is dimensionless time (multiples of $\mu$). We take the ratio of this service curve to that implied by the case of no-heterogeneity $1-e^{-\tau}$, to derive the discount factor associated with maximal heterogeneity, resulting in Figure 2 below. 

```{r Figure 2: service bounding curve, echo = F}

correction <- function(x){1-(1/(1+x))}
corr_diff <- function(x){(1-(1/(1+x)))/(1-exp(-x))}
...
p1 <- ggplot(data = data.frame(x=c(0,10)), aes(x)) + 
  ...
  xlab("Dimensionless time (mu(RT) multiples)") + 
  scale_x_continuous(breaks = seq(1,10,1))

p2 <- ggplot(data = data.frame(x=c(0,10)), aes(x)) + 
  stat_function(fun=corr_diff) + 
  theme_bw(base_size=15) + 
  ylab("Difference in correction factors") + 
  xlab("Dimensionless time (mu(RT) multiples)") + 
  scale_x_continuous(breaks = seq(1,10,1))
...
For a much less extreme case, we take the avoided GWP where phi = 1, corresponding to a fair amount of heterogeneity/uncertainty, (CV = 100%), but entirely reasonable. The implied distribution over residence times for an arbitrary $\mu=1$ is plotted in Figure 3a, and the resulting discount factor in Figure 3b. Our new equation, after non-dimensionalization, is:
$$ C_{avoided}(t) = C(0)\mu(1-(\frac{2}{2+\tau})^2)$$

```{r Figure 3: Service case study when phi is 1, echo = F}
...
alpha <- 1/phi + 2
beta <- mu*((1/phi)+1)
#beta/(alpha-1) checking should = mu :) 
...
p1 <- ggplot(data = data.frame(x=c(0,5)),aes(x)) + 
  ...
  xlab("Residence times") +
  ylab("Density") +
  theme_bw(base_size=15)
  

correction2 <- function(x){1-((2/(2+x))^2)}
exp_corr2 <- function(x){1-exp(-x)}
diff_corr <- function(x) {(1-((2/(2+x))^2))/(1-exp(-x))}

p2 <- ggplot(data = data.frame(x=c(0,10)), aes(x)) + 
  stat_function(fun=correction2) + 
  stat_function(fun = exp_corr2, color = "red") + 
  theme_bw(base_size=15) + 
  ylab("Correction factor") + 
  xlab("Dimensionless time (mu(RT) multiples)") + 
  scale_x_continuous(breaks = seq(1,10,1))
...
p3 <- ggplot(data = data.frame(x=c(0,10)), aes(x)) + 
  stat_function(fun=diff_corr) + 
  theme_bw(base_size=15) + 
  ylab("Proportional discount") + 
  xlab("Dimensionless time (mu(RT) multiples)") + 
  scale_x_continuous(breaks = seq(1,10,1))
