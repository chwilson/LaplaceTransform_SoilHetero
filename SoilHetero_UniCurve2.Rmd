...
For a much less extreme case, we take the avoided GWP where phi = 0.25, corresponding to a fair amount of heterogeneity/uncertainty, (CV = 50%), but entirely reasonable. The implied distribution over residence times for an arbitrary $\mu=1$ is plotted in Figure 3a, and the resulting discount factor in Figure 3b. Our new equation, after non-dimensionalization, is:
$$ C_{avoided}(t) = C(0)\mu(1-(\frac{5}{5+\tau})^5)$$

```{r Figure 3: Service case study when phi = 1, echo = F}
...
correction2 <- function(x){1-((5/(5+x))^5)}
...
*Figure 3: Correction factor to the service implied by the mean of the residence time distribution, when phi = 0.25, i.e. CV = 25%*
